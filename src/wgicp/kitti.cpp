...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::cout << "usage: wgicp_kitti /your/kitti/path/sequences/00/velodyne" << std::endl;
    return 0;
  }
  ...
  wgicp::dFastGICP<pcl::PointXYZ, pcl::PointXYZ> wgicp;
  ...
  for (int i = 1; i < kitti.size(); i++) {
    ...
    wgicp.setInputSource(source);

    // align and swap source and target cloud for next registration
    pcl::PointCloud<pcl::PointXYZ>::Ptr aligned(new pcl::PointCloud<pcl::PointXYZ>);
    wgicp.align(*aligned);
    wgicp.swapSourceAndTarget();

    // accumulate pose
    poses[i] = poses[i - 1] * wgicp.getFinalTransformation().cast<double>();
