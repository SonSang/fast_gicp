#ifndef WGICP_D_FAST_GICP_IMPL_HPP
#define WGICP_D_FAST_GICP_IMPL_HPP
...
dFastGICP<PointSource, PointTarget>::dFastGICP() {
#ifdef _OPENMP
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setNumThreads(int n) {
  num_threads_ = n;
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setCorrespondenceRandomness(int k) {
  k_correspondences_ = k;
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setRegularizationMethod(fast_gicp::RegularizationMethod method) {
  regularization_method_ = method;
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::swapSourceAndTarget() {
  input_.swap(target_);
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::clearSource() {
  input_.reset();
  source_covs_.clear();
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::clearTarget() {
  target_.reset();
  target_covs_.clear();
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setInputSource(const PointCloudSourceConstPtr& cloud) {
  if (input_ == cloud) {
    return;
  }
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setInputTarget(const PointCloudTargetConstPtr& cloud) {
  if (target_ == cloud) {
    return;
  }
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setSourceCovariances(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& covs) {
  source_covs_ = covs;
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::setTargetCovariances(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& covs) {
  target_covs_ = covs;
}

template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
...
template <typename PointSource, typename PointTarget>
void dFastGICP<PointSource, PointTarget>::update_correspondences(const Eigen::Isometry3d& trans) {
...
template <typename PointSource, typename PointTarget>
double dFastGICP<PointSource, PointTarget>::linearize(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b) {
...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    dtdx0.block<3, 3>(0, 0) = fast_gicp::skewd(transed_mean_A.head<3>());
...
bool dFastGICP<PointSource, PointTarget>::calculate_covariances(
...
#pragma omp parallel for num_threads(num_threads_) schedule(guided, 8)
  for (int i = 0; i < cloud->size(); i++) {
    ...
    if (regularization_method_ == fast_gicp::RegularizationMethod::NONE) {
      covariances[i] = cov;
    } else if (regularization_method_ == fast_gicp::RegularizationMethod::FROBENIUS) {
      double lambda = 1e-3;
      ...
      switch (regularization_method_) {
        default:
          ...
          abort();
        case fast_gicp::RegularizationMethod::PLANE:
          values = Eigen::Vector3d(1, 1, 1e-3);
          break;
        case fast_gicp::RegularizationMethod::MIN_EIG:
          values = svd.singularValues().array().max(1e-3);
          break;
        case fast_gicp::RegularizationMethod::NORMALIZED_MIN_EIG:
