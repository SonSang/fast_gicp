#ifndef FAST_GICP_DIFF_FAST_GICP_IMPL_HPP
#define FAST_GICP_DIFF_FAST_GICP_IMPL_HPP
...
DiffFastGICP<PointSource, PointTarget>::DiffFastGICP() {
#ifdef _OPENMP
  ...
  num_src_points_to_use_ = -1;
  src_weight_thresh_ = 0;

  reg_name_ = "DiffFastGICP";
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setNumThreads(int n) {
  num_threads_ = n;
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setCorrespondenceRandomness(int k) {
  k_correspondences_ = k;
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setRegularizationMethod(RegularizationMethod method) {
  regularization_method_ = method;
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::swapSourceAndTarget() {
  input_.swap(target_);
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::clearSource() {
  input_.reset();
  source_covs_.clear();
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::clearTarget() {
  target_.reset();
  target_covs_.clear();
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::backward() {
  // diff_source_weights_grads.resize(diff_source_weights_.size());
  // diff_target_weights_grads.resize(diff_target_weights_.size());

  // // Only compute grads w.r.t. source weights now
  // for (int i = 0; i < diff_source_weights_.size(); i++) {
  //   for (int j = 0; j < 4; j++) {
  //     for (int k = 0; k < 4; k++) {
  //       auto g = autodiff::derivatives(final_transformation_(j, k), autodiff::wrt(diff_source_weights_[i]));
  //       diff_source_weights_grads[i](j, k) = g;
  //     }
  //   }
  // }
}
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setInputSource(const PointCloudSourceConstPtr& cloud) {
  if (input_ == cloud) {
    return;
  }
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setInputTarget(const PointCloudTargetConstPtr& cloud) {
  if (target_ == cloud) {
    return;
  }
...
template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setSourceCovariances(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& covs) {
  source_covs_ = covs;
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setTargetCovariances(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& covs) {
  target_covs_ = covs;
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setSourceWeights(const std::vector<Scalar>& weights) {
  source_weights_ = weights;
  setSourceWeightThresh();
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setSourceWeightThresh() {
  if (num_src_points_to_use_ < 0)
    src_weight_thresh_ = 0;
  else {
    auto sorted_weights = source_weights_;
    std::sort(sorted_weights.begin(), sorted_weights.end());
    src_weight_thresh_ = sorted_weights.at(sorted_weights.size() - num_src_points_to_use_);
  }
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::setTargetWeights(const std::vector<Scalar>& weights) {
  target_weights_ = weights;
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  assert(num_src_points_to_use_ < 0 || num_src_points_to_use_ < input_->size());

  // Make [weights] to be differentiable inputs
  // diff_source_weights_.clear();
  // diff_target_weights_.clear();
  // diff_source_weights_.resize(source_weights_.size());
  // diff_target_weights_.resize(target_weights_.size());
  // for (int i = 0; i < source_weights_.size(); i++)
  //   diff_source_weights_[i] = source_weights_[i];
  // for (int i = 0; i < target_weights_.size(); i++)
  //   diff_target_weights_[i] = target_weights_[i];
  
  DiffLsqRegistration<PointSource, PointTarget>::computeTransformation(output, guess);
}

template <typename PointSource, typename PointTarget>
void DiffFastGICP<PointSource, PointTarget>::update_correspondences(const Eigen::Isometry3d& trans) {
...
#pragma omp parallel for num_threads(num_threads_) firstprivate(k_indices, k_sq_dists) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    // If the weight of this point is not large enough to be considered, omit it
    if (source_weights_[i] < src_weight_thresh_) {
    ...
    // Apply weight
    mahalanobis_[i] = mahalanobis_[i] * source_weights_[i];
  }
}

template <typename PointSource, typename PointTarget>
double DiffFastGICP<PointSource, PointTarget>::linearize(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b) {
...
template <typename PointSource, typename PointTarget>
double DiffFastGICP<PointSource, PointTarget>::compute_error(const Eigen::Isometry3d& trans) {
  double sum_errors = 0.0;
...
bool DiffFastGICP<PointSource, PointTarget>::calculate_covariances(
