#ifndef FAST_GICP_DIFF_FAST_GICP_ST_IMPL_HPP
#define FAST_GICP_DIFF_FAST_GICP_ST_IMPL_HPP
...
DiffFastGICPSingleThread<PointSource, PointTarget>::DiffFastGICPSingleThread() : DiffFastGICP<PointSource, PointTarget>() {
  this->reg_name_ = "DiffFastGICPSingleThread";
  this->num_threads_ = 1;
}

template <typename PointSource, typename PointTarget>
DiffFastGICPSingleThread<PointSource, PointTarget>::~DiffFastGICPSingleThread() {}

template <typename PointSource, typename PointTarget>
void DiffFastGICPSingleThread<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  anchors_.clear();
  DiffFastGICP<PointSource, PointTarget>::computeTransformation(output, guess);
}

template <typename PointSource, typename PointTarget>
void DiffFastGICPSingleThread<PointSource, PointTarget>::update_correspondences(const Eigen::Isometry3d& x) {
...
template <typename PointSource, typename PointTarget>
double DiffFastGICPSingleThread<PointSource, PointTarget>::linearize(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b) {
  if (H && b) {
...
template <typename PointSource, typename PointTarget>
double DiffFastGICPSingleThread<PointSource, PointTarget>::compute_error(const Eigen::Isometry3d& trans) {
